...
export default function StudyGroupNotificationBell() {
  ...
  const markRead = async (item) => { if (!item.readAt) { const now = new Date().toISOString(); setItems((current) => current.map((entry) => entry._id === item._id ? { ...entry, readAt: now } : entry)); setUnread((count) => Math.max(0, count - 1)); scheduleCleanup(); await api.post(`/api/notifications/${item._id}/read`).catch(load); } if (item.groupId) navigate(`/dashboard/groups/${item.groupId}?tab=Chat`); };

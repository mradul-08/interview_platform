...
export default function StudyGroupAccessGate({ children }) {
  ...
  return <main className="sg-app"><div className="sg-main"><section className={`sg-card sg-access-state ${state === "pending" ? "is-pending" : ""}`} aria-live="polite"><span className="sg-eyebrow">GROUP ACCESS</span><div className="sg-access-avatar">{group?.avatarText || group?.name?.[0] || "S"}</div><h1>{group?.name || "Study group"}</h1><p>{state === "pending" ? "Your request is pending approval. We will keep checking while you wait." : "Join this group to view its members, discussions, chat, sessions, and tasks."}</p>{state !== "pending" && <button className="sg-btn accent" type="button" onClick={join} disabled={joining}>{joining ? "Joining..." : "Join group"}</button>}{state === "pending" && <span className="sg-access-pending">Waiting for owner approval</span>}<Link className="sg-btn" to="/dashboard/groups">Back to groups</Link>{error && <p className="sg-error-text" role="alert">{error}</p>}</section></div></main>;
}

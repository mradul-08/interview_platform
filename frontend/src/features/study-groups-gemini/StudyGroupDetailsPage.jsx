...
export default function StudyGroupDetailsPage() {
  ...
  const load = useCallback(async () => { try { setData(await getDashboard(groupId)); setError(""); } catch (err) { setError(err.response?.data?.message || "This group could not be loaded."); } finally { setLoading(false); } }, [groupId, setData]);

...
function SessionViewV2({ session, question, questionIndex, elapsed, isExam, hasCountdown, selectedAnswer, setSelectedAnswer, result, submitting, markedQuestions, questionStatuses, statusCounts, bookmarked, error, onSubmit, onSkip, onNext, onFinish, onMark, onBookmark, onQuestion, onExit }) {
  ...
  return <div className="apt-session-shell"><header className="apt-session-header"><button className="apt-icon-btn" onClick={onExit} aria-label="Exit session">←</button><div><strong>{isExam ? "Mock test" : "Practice session"}</strong><span>{question.category || "Aptitude"} · {question.topic}</span></div><div className="apt-session-progress"><span>Question {questionIndex + 1} / {total}</span><div>{Array.from({ length: total }).map((_, index) => <i key={index} className={index < questionIndex ? "done" : index === questionIndex ? "current" : ""} />)}</div></div><div className={`apt-timer ${hasCountdown && elapsed < 60 ? "danger" : ""}`}>◷ {formatTime(elapsed)}</div><button className="apt-btn secondary" onClick={onExit}>Exit</button></header><div className="apt-session-body"><article className="apt-question-panel"><div className="apt-question-meta"><span className={`apt-difficulty ${(question.difficulty || "medium").toLowerCase()}`}>{question.difficulty}</span><span>{question.expectedTime}s target</span><button className={bookmarked ? "active apt-inline-btn" : "apt-inline-btn"} onClick={onBookmark}>◇ {bookmarked ? "Saved" : "Save"}</button><button className="apt-inline-btn" onClick={onMark}>{markedQuestions.has(questionIndex) ? "★ Marked" : "☆ Mark for review"}</button></div><h1>{question.question}</h1><div className="apt-options" role="radiogroup" aria-label="Answer choices">{question.options.map((option) => { const picked = selectedAnswer === option.key; const correct = result?.isCorrect && picked; const wrong = result && picked && !result.isCorrect; return <button key={option.key} role="radio" aria-checked={picked} className={`${picked ? "selected" : ""} ${correct ? "correct" : ""} ${wrong ? "wrong" : ""}`} disabled={Boolean(result) || submitting || expired} onClick={() => setSelectedAnswer(option.key)}><span>{option.key}</span><strong>{option.text}</strong>{correct && <b>✓</b>}</button>; })}</div>{expired && <div className="apt-alert">Time is up. Submit the session to see your result.</div>}{result && <div className={`apt-feedback ${result.isCorrect ? "success" : "error"}`}><strong>{result.isCorrect ? "Correct — nice work" : "Not quite this time"}</strong><p>{result.explanation}</p>{result.shortTrick && <small><b>Shortcut:</b> {result.shortTrick}</small>}</div>}{error && <div className="apt-alert">{error}</div>}<footer className="apt-question-actions"><button className="apt-btn ghost" onClick={onSkip} disabled={submitting || Boolean(result) || expired}>Skip question</button>{isExam ? <button className="apt-btn primary" onClick={selectedAnswer ? onSubmit : onNext} disabled={submitting || expired}>{questionIndex + 1 === total ? "Submit test" : "Save & next"} <span>→</span></button> : <button className="apt-btn primary" onClick={result ? onNext : onSubmit} disabled={submitting || expired || (!result && !selectedAnswer)}>{result ? "Next question" : "Submit answer"} <span>→</span></button>}</footer></article><aside className="apt-question-sidebar"><div className="apt-side-card"><div className="apt-side-heading"><strong>Question navigator</strong><small>{statusCounts.answered} solved · {statusCounts.skipped} skipped · {statusCounts.unanswered} remaining</small></div><div className="apt-navigator">{Array.from({ length: total }).map((_, index) => <button key={index} aria-label={`Question ${index + 1}`} className={statusClass(index)} onClick={() => onQuestion(index)}>{String(index + 1).padStart(2, "0")}</button>)}</div><div className="apt-legend"><span><i className="solved" /> Solved ({statusCounts.answered})</span><span><i className="skipped" /> Skipped ({statusCounts.skipped})</span><span><i className="unsolved" /> Unsolved ({statusCounts.unanswered})</span><span><i className="review" /> Review</span></div></div><div className="apt-side-card apt-side-note"><strong>{isExam ? "Mock test mode" : "Practice mode"}</strong><p>{isExam ? "Explanations unlock after you submit the test." : "Answer freely; feedback appears immediately."}</p></div><button className="apt-submit-test" onClick={onFinish}>{isExam ? "Submit test" : "Finish session"}</button></aside></div></div>;
}
